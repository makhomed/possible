...
│   ├─centos-opt  253:1    0    8G  0 lvm   /opt
│   ├─centos-tmp  253:2    0    8G  0 lvm   /tmp
│   └─centos-var  253:3    0    8G  0 lvm   /var
...
class Parted:
    def __init__(self, c, devices):
        ...
        self.offset = GPT_SIZE_IN_SECTORS + BIOS_GRUB_SIZE_IN_SECTORS
        self.part_index = 1
...
class PartMan:
    def __init__(self, hosts):
        ...
        host = hosts[0]
        self.c = Context(host)
        self.uefi_size_in_gib = int(self.c.var('uefi', '1G').rstrip('G'))
    ...
    def mkpart(self):
        ...
        parted.mkpart('for uefi /dev/md0', gib_to_sectors(self.uefi_size_in_gib), mdraid=True, lvm=False)
        parted.mkpart('for boot /dev/md1', gib_to_sectors(self.boot_size_in_gib), mdraid=True, lvm=False)
        parted.mkpart('for lvm  /dev/md2', gib_to_sectors(self.lvm_size_in_gib), mdraid=True, lvm=True)
        parted.mkpart('for zfs zpool', -1, mdraid=False, lvm=False)
        parted.run()

        for device in self.devices:
            self.c.run(f"parted -s /dev/{device} -- set 1 raid on")
            self.c.run(f"parted -s /dev/{device} -- set 2 raid on")
            self.c.run(f"parted -s /dev/{device} -- set 3 raid on")

        time.sleep(1)

        self.c.run(f"mdadm --create /dev/md0 --level=mirror --raid-devices={len(self.devices)} {self.parts(1)} --metadata=1.0")
        self.c.run(f"mdadm --create /dev/md1 --level=mirror --raid-devices={len(self.devices)} {self.parts(2)} --metadata=1.2")
        self.c.run(f"mdadm --create /dev/md2 --level=mirror --raid-devices={len(self.devices)} {self.parts(3)} --metadata=1.2")

        self.c.run("pvcreate /dev/md2")

        self.c.run("vgcreate centos /dev/md2")
    ...
    def rmpart(self):
        ...
        self.c.run("pvremove -f /dev/md2", can_fail=True)

        self.c.run("mdadm --stop /dev/md0", can_fail=True)
        self.c.run(f"mdadm --zero-superblock {self.parts(1)}", can_fail=True)
        self.c.run("mdadm --stop /dev/md1", can_fail=True)
        self.c.run(f"mdadm --zero-superblock {self.parts(2)}", can_fail=True)
        self.c.run("mdadm --stop /dev/md2", can_fail=True)
